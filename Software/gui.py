...
class DashboardGUI:
    ...
    def draw_arm(self, axes_deg):
        ...
        if not c.find_withtag("arm_rect1"):
            # Oczyszczanie przed pierwszym rysowaniem dla pewności
            c.delete("all")
            c.create_rectangle(cx-30, cy, cx+30, cy+15, fill="#333", outline="#555", width=2, tags="arm_rect1")
            c.create_rectangle(cx-20, cy-5, cx+20, cy, fill="#444", outline="#555", tags="arm_rect2")
            c.create_polygon(cx-32, cy+18, cx-42, cy+25, cx-28, cy+28, fill=col_L, outline=col_L, tags="arm_polyL")
            c.create_polygon(cx+32, cy+18, cx+42, cy+25, cx+28, cy+28, fill=col_R, outline=col_R, tags="arm_polyR")
            ...
            c.create_oval(x1-3, y1-3, x1+3, y1+3, fill="#00ffff", outline="", tags="arm_oval1")
            c.create_oval(x2-3, y2-3, x2+3, y2+3, fill="#00ffff", outline="", tags="arm_oval2")
            c.create_oval(x3-3, y3-3, x3+3, y3+3, fill="#00ffff", outline="", tags="arm_oval3")
        else:
            c.coords("arm_rect1", cx-30, cy, cx+30, cy+15)
            c.coords("arm_rect2", cx-20, cy-5, cx+20, cy)
            c.coords("arm_polyL", cx-32, cy+18, cx-42, cy+25, cx-28, cy+28)
            c.itemconfig("arm_polyL", fill=col_L, outline=col_L)
            c.coords("arm_polyR", cx+32, cy+18, cx+42, cy+25, cx+28, cy+28)
            c.itemconfig("arm_polyR", fill=col_R, outline=col_R)
            ...
            c.coords("arm_oval1", x1-3, y1-3, x1+3, y1+3)
            c.coords("arm_oval2", x2-3, y2-3, x2+3, y2+3)
            c.coords("arm_oval3", x3-3, y3-3, x3+3, y3+3)
    ...
    def draw_scissor_tip(self, rotation_deg, jaw_angle):
        ...
        if not c.find_withtag("tip_text_rot"):
            ...
            c.create_oval(cx-20, cy-20, cx+20, cy+20, outline="#555", width=2, tags="tip_oval1")
            c.create_line(cx, cy-20, cx, cy+20, fill="#333", width=2, tags="tip_line1")
            c.create_polygon(*p1, fill="#ccc", outline="black", tags="tip_poly1")
            c.create_polygon(*p2, fill="#ccc", outline="black", tags="tip_poly2")
            c.create_oval(cx-4, cy-4, cx+4, cy+4, fill="#ff6600", tags="tip_oval2")
        else:
            c.itemconfig("tip_text_rot", text=f"Rotacja: {rotation_deg:.0f}°")
            c.coords("tip_poly1", *p1)
            c.coords("tip_poly2", *p2)