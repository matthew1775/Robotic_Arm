...
class DashboardGUI:
    ...
    def update_interface(self):
        ...
        # ⚡ Bolt: Memoize UI application state to conditionally skip expensive Tkinter redraws or label configurations when data hasn't changed.
        current_led_state = (self.state.mqtt_status_text, self.state.ping_ground_ok, self.state.ping_router_ok, self.state.ping_broker_ok)
        if current_led_state != self._last_led_states:
            ...
            self._last_led_states = current_led_state

        # Rysowanie 8 kwadratów
        for i in range(8):
            target_deg = self.state.target_joints_deg[i]
            actual_deg = self.state.actual_joints_deg[i]
            current_joint_state = (target_deg, actual_deg)

            if current_joint_state != self._last_joint_states[i]:
                ...
                self._last_joint_states[i] = current_joint_state
