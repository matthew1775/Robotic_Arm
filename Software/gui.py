...
class DashboardGUI:
    ...
    def draw_arm(self, axes_deg):
        # OPTIMIZATION: Instead of c.delete("all") and recreating shapes (which causes Tcl/Tk ID
        # incrementing and memory leaks in high-frequency loops), we use find_withtag and
        # c.coords()/c.itemconfig() to reuse existing canvas items.
        c = self.canvas_arm
        ...
        if not c.find_withtag("base_rect1"):
            ...
            c.create_text(cx, cy+35, text=f"Obrót: {base_rot_val:.0f}°", fill="#888", font=("Arial", 8), tags="text_rot")
            ...
            c.create_oval(0,0,0,0, fill="#00ffff", outline="", tags="joint1")
            c.create_oval(0,0,0,0, fill="#00ffff", outline="", tags="joint2")
            c.create_oval(0,0,0,0, fill="#00ffff", outline="", tags="joint3")
        else:
    ...
    def draw_scissor_tip(self, rotation_deg, jaw_angle):
        # OPTIMIZATION: Reuse canvas elements via tags to prevent memory leaks in the render loop
        c = self.canvas_tip
        cx, cy = 125, 125 
        ...
        if not c.find_withtag("text_rot"):
            c.create_text(cx, cy+70, text=f"Rotacja: {rotation_deg:.0f}°", fill="#888", font=("Arial", 10, "bold"), tags="text_rot")
            c.create_oval(cx-20, cy-20, cx+20, cy+20, outline="#555", width=2, tags="base_circle")
            c.create_line(cx, cy-20, cx, cy+20, fill="#333", width=2, tags="base_line")
            ...
            c.create_oval(cx-4, cy-4, cx+4, cy+4, fill="#ff6600", tags="center_dot")
        else:
            c.itemconfig("text_rot", text=f"Rotacja: {rotation_deg:.0f}°")
